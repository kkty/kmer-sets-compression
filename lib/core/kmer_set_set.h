...

    {
      ...
      for (int i = 0; i < n; i++) {
        boost::asio::post(pool, [&, i] {
          bloom_filters[i] =
              kmer_sets_compact_[i].GetBloomFilter(bloom_filter_size);
        });
      }
